...
export default function TopPerformerBox({ stocks }: { stocks: StockInfo[] }) {
  ...
  const topPerformers = useMemo(() => {
    ...
    return [...performanceData].sort((a, b) => {
      ...
      if (timeframe === "week") return bW - aW;
      if (timeframe === "month") return bM - aM;
      return bY - aY;
    }).slice(0, 10);
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 w-full z-10">
        <div className="flex items-center gap-3">
          <div className="h-12 w-12 rounded-xl bg-gradient-to-br from-emerald-500 to-emerald-800 flex items-center justify-center shadow-lg shadow-emerald-900/50 shrink-0">
            <Award className="w-6 h-6 text-emerald-50" />
          </div>
          <div>
            <h3 className="text-xl font-black text-white tracking-tight">Top Performers</h3>
            <p className="text-emerald-400 font-bold text-[10px] uppercase tracking-widest flex items-center gap-1.5 mt-0.5">
              <TrendingUp className="w-3 h-3" /> Momentum Leaders
            </p>
          </div>
        </div>
      ...
      <div className="flex overflow-x-auto space-x-4 pb-2 pt-1 z-10 snap-x custom-scrollbar">
        {topPerformers.map((stock, index) => {
          ...
          return (
            <div 
              key={stock.symbol} 
              className="min-w-[200px] max-w-[200px] bg-zinc-950/50 hover:bg-zinc-900 border border-zinc-850 hover:border-emerald-500/30 rounded-2xl p-5 flex flex-col gap-4 transition-all duration-300 snap-start group shrink-0 relative overflow-hidden"
            >
              <div className="absolute top-0 right-0 w-20 h-20 bg-emerald-500/10 rounded-full blur-xl -translate-y-1/2 translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none" />
              ...
              <div>
                <h4 className="text-xl font-black text-white group-hover:text-emerald-400 transition-colors tracking-tight">{stock.symbol.split('.')[0]}</h4>
              ...
              <div className="flex items-baseline gap-1 mt-auto pt-2">
                <span className="text-2xl font-black text-emerald-400">+{displayPerf.toFixed(2)}%</span>
              </div>
            </div>
          );
        })}
